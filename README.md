...
**On/Off button**
The on/off button is used to **activate and deactivate Bitcointags**. When Bitcointags are **disabled**, there is **no listening** on the page or **calling the currency APIs**, so **Bitcointags are not active**.
...
#### Normal tag
The **normal tag** is displayed if everything is running fine, or if an error occurs during an currency APIs call but the program already has older API data. You can see the **normal tag** below.
...
#### Error tag
If the **Currency APIs response** is empty, an **error** tag will be displayed. This indicates that an error occurred when the **API was first called**. Please refer to the [Errors](docs/DOCUMENTATION.md#errors) section of the [documentation](docs/DOCUMENTATION.md) for a more detailed explanation of this issue. You can see the **error tag** below.

![Bitcointags error tag](docs/img/tag_2.svg)

**Error code**
Error code is the **HTTP status code** returned by the **Currency APIs** call promise. For details on the use of status codes in Bitcointags, see the [Errors](#errors) section of the [documentation](docs/DOCUMENTATION.md).
